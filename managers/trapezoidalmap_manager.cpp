...
void TrapezoidalMapManager::addSegmentToTrapezoidalMap(const cg3::Segment2d& segment)
{
    ...
    // Clear the last trapezoid found through the query from the bright blue color
    if (lastTrapezoidFound < drawableTrapezoidalMap.getTrapezoids().size()) {
        drawableTrapezoidalMap.setRandomColor(lastTrapezoidFound);
        lastTrapezoidFound = std::numeric_limits<size_t>::max();
    }

    drawableTrapezoidalMap.addSegment(segment);
    drawableTrapezoidalMap.updateColor();
    updateCanvas();
...
void TrapezoidalMapManager::queryTrapezoidalMap(const cg3::Point2d& queryPoint)
{
    ...
    lastTrapezoidFound = drawableTrapezoidalMap.pointLocationQuery(queryPoint);
    drawableTrapezoidalMap.setColor(lastTrapezoidFound, {0, 0, 1}); // Highlights the found trapezoid
    updateCanvas();
